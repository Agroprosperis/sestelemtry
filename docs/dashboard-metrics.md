...
synthetic-ключами на всіх пресетах, але відповідає на них різний
конвеєр. Вікно до `maxEnergyFlowWindow` (36 годин) рахується на льоту
алокатором по сирих семплах. Ширші вікна — місяць і рік — сумуються з
денних підсумків у `economics_daily`, які демон `economics-recompute`
порахував тим самим алокатором по одному дню (жива прогонка по місяцю
зайняла б хвилини). Яким шляхом отримані числа, видно з
`flows_meta.source` (`allocator` або `daily_cache`); для кеша там ще й
`days_covered` / `days_expected`, тому неповний період можна показати
як неповний, а не тихо занизити. Через це перетоки за місяць/рік
залежать від того, чи працює демон економіки, а внесок сьогоднішнього
дня в місячну суму відстає на один його інтервал.

У відповіді з’являється поле `flows: { pv_to_ess_kwh, grid_to_ess_kwh,
ess_to_load_kwh, ess_to_grid_kwh }`, яке `flowsFromTotals`
(`web/src/dashboard/transforms/flows.ts`) перетворює у графік. Жива
діаграма `EnergyFlowLive.tsx` живиться `/api/v1/current` і
synthetic-ключів не використовує.

### План проти факту (прогноз генерації)

Картка підсумку порівнює факт із прогнозом n8n на всіх пресетах. День
сумує ту саму погодинну криву, яку вже малює на графіку. Місяць і рік
беруть суму з `/api/v1/pv-plan-summary` (`internal/api/pvplan.go`):
потік віддає один день на запит, тому рік — це 365 запитів, і денні
підсумки кешуються в `pv_plan_daily`. Прогноз минулого дня не
змінюється, тому його запис у кеші вічний; сьогоднішній оновлюється раз
на 30 хвилин, а день, для якого прогнозу немає, запам’ятовується як
промах і перепитується раз на добу. Відсутні дні добираються під час
запиту в межах бюджету часу, тому холодний рік заповнюється за перші
кілька опитувань.

Кінець вікна обрізається по сьогоднішній день: потік прогнозує приблизно
на два тижні вперед, і якщо взяти план на весь місяць, то факт за
пройдену частину виглядав би провалом. Коли `days_covered` менше за
`days_expected` — частина періоду старша за історію самого потоку, і
картка пише це поруч із прогнозом, а не подає неповний план як повний.
Адресу потоку можна перевизначити через `PV_FORECAST_WEBHOOK_URL`.
