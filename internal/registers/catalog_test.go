...
func TestEffectivePollModeDefaultsToFast(t *testing.T) {
	cases := []struct {
		name string
		raw  PollMode
		want PollMode
		...
		{"empty", "", PollFast},
		{"fast explicit", PollFast, PollFast},
		{"slow explicit", PollSlow, PollSlow},
		{"unknown", PollMode("weird"), PollFast},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := Entry{PollMode: c.raw}.EffectivePollMode()
			if got != c.want {
...
func TestFilterByModeSplitsFastAndSlow(t *testing.T) {
	all := []ResolvedEntry{
		{Entry: Entry{MetricKey: "a"}},
		{Entry: Entry{MetricKey: "b", PollMode: PollFast}},
		{Entry: Entry{MetricKey: "c", PollMode: PollSlow}},
		{Entry: Entry{MetricKey: "d", PollMode: PollSlow}},
	}
	fast := FilterByMode(all, PollFast)
	slow := FilterByMode(all, PollSlow)
	if len(fast) != 2 || fast[0].MetricKey != "a" || fast[1].MetricKey != "b" {
		t.Fatalf("fast subset wrong: %+v", fast)
	}
	if len(slow) != 2 || slow[0].MetricKey != "c" || slow[1].MetricKey != "d" {
		t.Fatalf("slow subset wrong: %+v", slow)
	}
}

func TestHuaweiCatalogServiceTimeRegisters(t *testing.T) {
	cat, err := Load("../../registers/huawei_smartlogger.yaml")
	if err != nil {
		t.Fatalf("Load error: %v", err)
	}
	...
	want := map[string]PollMode{
		"device_epoch_seconds":            PollFast,
		"device_timezone_offset_seconds":  PollSlow,
		"device_dst_state":                PollSlow,
		"device_dst_offset_minutes":       PollSlow,
		"device_local_epoch_seconds":      PollSlow,
	}
	for _, e := range resolved {
		if mode, ok := want[e.MetricKey]; ok {
			if e.EffectivePollMode() != mode {
				t.Fatalf("entry %q poll_mode = %q, want %q", e.MetricKey, e.EffectivePollMode(), mode)
			}
			delete(want, e.MetricKey)
		}
	}
	if len(want) != 0 {
		t.Fatalf("missing service time registers: %+v", want)
	}
}
